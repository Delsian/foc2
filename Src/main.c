...
struct pwm_device *pwm_dev[2];
struct foc_motor *motor[2];
// static mt6701_t encoder_motor0;  // Not used in Motor 1 only configuration
mt6701_t encoder_motor1;
...
bool encoder_enabled[2] = {false, false};  /* Track encoder status per motor */
...
int main(void)
{
    ...
    while (1) {
        ...
        if (command & CMD_VEL_INC) {
            command &= ~CMD_VEL_INC;
            target_rpm += 10.0f;
            if (target_rpm > 500.0f) target_rpm = 500.0f;
            if (!velocity_mode) {
                foc_velocity_enable(motor[1], FOC_VELOCITY_OPEN_LOOP, target_rpm, amplitude, 1000.0f, 7);
                velocity_mode = true;
        ...
        if (command & CMD_TOGGLE_ENCODER) {
            command &= ~CMD_TOGGLE_ENCODER;
            if (encoder_enabled[1]) {
                ...
                if (!encoder1_ok) {
                    ...
                    printf("Check I2C1 connection and encoder power.\n");
                    printf("Expected: Encoder on I2C1, address 0x06\n");
                } else {
        ...
        if (command & CMD_INFO) {
            ...
            printf("Current: %d mA (limit: %d A)\n", (int)(current_a * 1000), (int)motor[1]->current_cfg.current_limit_a);
